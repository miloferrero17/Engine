...
from app.Model.field import Field
from app.Model.enums import *
from app.Model.exceptions import DatabaseError, UniqueConstraintError, ValidationError, BaseModelError, MissingUniqueFieldError, RecordNotFoundError
from app.Model.tools import get_fields_and_params, snake_to_camel
from app.Model.validators import validate
import warnings
warnings.filterwarnings("ignore", message=".*found in sys.modules.*")
...
class BaseModel:
    def __init__(self, table_name: str, data: Dict[str, Any]) -> None:
        self.table_name = table_name
        self.__data = data
        # Se define la URL base para la tabla (usando la REST API de Supabase)
        self.base_url = f"{SUPABASE_URL}/rest/v1/{self.table_name}"
        self.headers = {
            "apikey": SUPABASE_API_KEY,
            "Authorization": f"Bearer {SUPABASE_API_KEY}",
    ...
    def add(self) -> int:
        ...
        """
        try:
            fields, params = get_fields_and_params(self.__data,
                                                   for_update=False)
            if not fields:
                raise Exception("No hay datos para actualizar.")
            ...
            if not response_data:
                raise Exception("No se devolvió registro creado.")
            new_record = response_data[0]
            # Se asume que el ID viene en "id" o similar; ajustá según corresponda
            return new_record.get("id") or new_record.get("user_id") or 0

        except Exception as e:
            raise DatabaseError(
                f"Error al crear registro en {self.table_name}: {e}")

    def get(self,
            field: str,
            value: Any,
            order_field: str = None) -> Optional[List[Any]]:
        """
        Obtiene uno o más registros filtrando por un campo.
        """
        try:
            if field not in self.__data:
                raise ValidationError(f"El campo {field} no existe.",
                                      field=field,
                                      value=value)
            if not validate(value, self.__data[field].data_type,
                            self.__data[field].optional):
                raise ValidationError(
                    f"El {field} debe ser de tipo {self.__data[field].data_type}.",
                    field=field,
                    value=value)
            url = f"{self.base_url}?{field}=eq.{value}"
            if order_field:
            ...
            if r.status_code >= 400:
                raise DatabaseError(
                    f"Error al obtener registro por {field} en {self.table_name}: {r.status_code}, {r.text}"
                )
            records = r.json()
            if not records:
                return None
            ...
            raise DatabaseError(
                f"Error al obtener registro por {field} en {self.table_name}: {e}"
            )

    # Puedes agregar aquí otros métodos como get_all, update, delete, etc.

    def get_all(self, order_field: str = None) -> List[Any]:
        """
        Obtiene todos los registros de la tabla.
    
        :param order_field: Campo por el cual ordenar (opcional).
        :return: Lista de objetos (convertidos al registro correspondiente) o una lista vacía si no hay registros.
        :raises DatabaseError: Si ocurre algún error durante la consulta.
        """
        try:
            url = f"{self.base_url}?select=*"
            if order_field:
            ...
            if r.status_code >= 400:
                raise DatabaseError(
                    f"Error al obtener todos los registros de {self.table_name}: {r.status_code}, {r.text}"
                )
            records = r.json()
            if not records:
                return []
            ...
            raise DatabaseError(
                f"Error al obtener todos los registros de {self.table_name}: {e}"
            )

    def get_with_multiple_fields(
            self,
            fields: Dict[str, Any],
            order_field: str = None) -> Optional[List[Any]]:
        """
        Obtiene registros filtrando por múltiples campos.

        :param fields: Diccionario con los filtros (campo: valor).
        :param order_field: Campo por el cual ordenar los resultados (opcional).
        :return: Lista de objetos convertidos a la clase Register correspondiente o None si no se encuentran registros.
        :raises DatabaseError: Si ocurre algún error durante la consulta.
        """
        try:
            filter_str = ""
            ...
            if r.status_code >= 400:
                raise DatabaseError(
                    f"Error al obtener registros en {self.table_name}: {r.status_code}, {r.text}"
                )
            records = r.json()
            if not records:
                return None
            ...
            raise DatabaseError(
                f"Error al obtener registros con múltiples campos en {self.table_name}: {e}"
            )

    def update(self, unique_field_name: str, unique_field_value: Any) -> None:
        """
        Actualiza un registro existente identificándolo mediante un campo único.

        :param unique_field_name: Nombre del campo único que identifica el registro (por ejemplo, "email").
        :param unique_field_value: Valor del campo único para identificar el registro a actualizar.
        :raises DatabaseError: Si ocurre algún error durante la actualización.
        :raises UniqueConstraintError: Si se intenta actualizar con un valor que debe ser único y ya existe.
        :raises RecordNotFoundError: Si no se encuentra el registro a actualizar.
        """
        try:
            fields, params = get_fields_and_params(self.__data,
                                                   for_update=True)
            if not fields:
                raise ValidationError("No hay datos para actualizar.",
                                      field=None,
                                      value=None)
            # Remover el " = %s" de cada campo para que las claves sean solo el nombre de la columna
            actual_fields = [field.replace(" = %s", "") for field in fields]
            payload = {
                field: value
                for field, value in zip(actual_fields, params)
            }
            url = f"{self.base_url}?{unique_field_name}=eq.{unique_field_value}"
            r = requests.patch(url, headers=self.headers, json=payload)
            if r.status_code >= 400:
                ...
                for field, field_obj in self.__data.items():
                    if field_obj.unique and str(field_obj.value) in error_text:
                        raise UniqueConstraintError(
                            f"El {field} ya está en uso.",
                            field=field,
                            value=field_obj.value)
                raise DatabaseError(
                    f"Error al actualizar registro en {self.table_name}: {r.status_code}, {r.text}"
                )
            if not r.json():
                raise RecordNotFoundError(
                    f"No se encontró el registro con {unique_field_name}={unique_field_value} en {self.table_name}.",
            ...
            raise DatabaseError(
                f"Error al actualizar registro en {self.table_name}: {e}")
    ...
    def _fetch_one(self, query: str) -> dict:
        ...
        url = os.environ.get("SUPABASE_URL") + f"/rest/v1/{self._table}?{query}"
        headers = {
            "apikey": os.environ.get("SUPABASE_KEY"),
            "Authorization": f"Bearer {os.environ.get('SUPABASE_KEY')}",
            "Content-Type": "application/json"
    ...
    def delete(self, field: str, value: Any) -> None:
        """
        Elimina uno o varios registros filtrando por un campo.

        :param field: Nombre del campo por el cual filtrar.
        :param value: Valor que debe tener el campo para eliminar el registro.
        :raises DatabaseError: Si ocurre algún error durante la eliminación.
        """
        try:
            # Verifica que el campo exista en la estructura definida
            if field not in self.__data:
                raise ValidationError(f"El campo {field} no existe.",
                                      field=field,
                                      value=value)
            # Valida que el valor cumpla con el tipo esperado
            if not validate(value, self.__data[field].data_type,
                            self.__data[field].optional):
                raise ValidationError(
                    f"El {field} debe ser de tipo {self.__data[field].data_type}.",
                    field=field,
                    value=value)
            # Construye la URL con el filtro para eliminar el registro
            url = f"{self.base_url}?{field}=eq.{value}"
            r = requests.delete(url, headers=self.headers)
            if r.status_code >= 400:
                raise DatabaseError(
                    f"Error al eliminar registro en {self.table_name}: {r.status_code}, {r.text}"
                )
            # Opcional: verificar si se eliminó algún registro
            if not r.json():
                raise DatabaseError(
                    f"No se encontró registro con {field}={value} en {self.table_name}."
                )
        except Exception as e:
            raise DatabaseError(
                f"Error al eliminar registro en {self.table_name}: {e}")
